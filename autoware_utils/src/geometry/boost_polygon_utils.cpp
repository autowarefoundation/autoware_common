  ...
  const geometry_msgs::msg::Pose & pose, const autoware_perception_msgs::msg::Shape & shape)
{
  ...
  if (shape.type == autoware_perception_msgs::msg::Shape::BOUNDING_BOX) {
    const auto point0 =
      autoware_utils::calcOffsetPose(pose, shape.dimensions.x / 2.0, shape.dimensions.y / 2.0, 0.0)
        .position;
    const auto point1 =
      autoware_utils::calcOffsetPose(pose, -shape.dimensions.x / 2.0, shape.dimensions.y / 2.0, 0.0)
        .position;
    const auto point2 = autoware_utils::calcOffsetPose(
...
autoware_utils::Polygon2d toPolygon2d(const autoware_perception_msgs::msg::DetectedObject & object)
{
  return autoware_utils::toPolygon2d(object.kinematics.pose_with_covariance.pose, object.shape);
}

autoware_utils::Polygon2d toPolygon2d(const autoware_perception_msgs::msg::TrackedObject & object)
{
  return autoware_utils::toPolygon2d(object.kinematics.pose_with_covariance.pose, object.shape);
}

autoware_utils::Polygon2d toPolygon2d(const autoware_perception_msgs::msg::PredictedObject & object)
{
  return autoware_utils::toPolygon2d(
  ...
  const double base_to_rear, const double width)
{
  Polygon2d polygon;
  const auto point0 =
    autoware_utils::calcOffsetPose(base_link_pose, base_to_front, width / 2.0, 0.0).position;
  const auto point1 =
    autoware_utils::calcOffsetPose(base_link_pose, base_to_front, -width / 2.0, 0.0).position;
  const auto point2 =
    autoware_utils::calcOffsetPose(base_link_pose, -base_to_rear, -width / 2.0, 0.0).position;
  const auto point3 =
    autoware_utils::calcOffsetPose(base_link_pose, -base_to_rear, width / 2.0, 0.0).position;
